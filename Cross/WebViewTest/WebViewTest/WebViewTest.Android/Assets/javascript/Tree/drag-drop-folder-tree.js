...
************************************************************************************************************/	
	...
	JSDragDropTree.prototype = {
		...
		*/
		...
		showSelectedNode: function(inputId)
		{
			...
			for(var no=elements.length-1; no>=0; no--){
				var subItems = elements[no].getElementsByTagName('UL');
				if(subItems.length>0 && subItems[0].style.display!='block'){
					JSTreeObj.showHideNode(false,elements[no].id);
				}			
			}
		}
		,
		...
		,
		initTree : function(selectIdNode)
		{
			JSTreeObj = this;
			...
			if(selectIdNode){
				this.showSelectedNode(selectIdNode);	
			}						
