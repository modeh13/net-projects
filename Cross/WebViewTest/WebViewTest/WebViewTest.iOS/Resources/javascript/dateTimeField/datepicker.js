...
function getNDutyDays(){
	return nDutyDays;
}

function setNDutyDays(nonDutyDays){
	this.nDutyDays = nonDutyDays;
}

Control.DatePicker = Class.create();
Control.DatePicker.prototype = {
    initialize: function(element, options, weekendDays, nonDutyDays)
    {
        ...
        if(weekendDays != null){
            this.options.weekend = weekendDays;
        }
        this.datepicker = new Control.DatePickerPanel(this.options, nonDutyDays);
        ...
        if (!this.options.disabled)
        {
            if (this.options.icon)
            {
                ...
                var topOffset = Prototype.Browser.IE ? '1px' : '3px';
                Element.setStyle(this.icon, {'position': 'relative', 'width': '15px', 'height': '15px', 'left': '-18px', 'top': topOffset});
    ...
    show: function ()
    {
        if (!this.pickerActive)
        {
            ...
            var pickerTop = Prototype.Browser.IE ? (pos[1] + dim.height) + 'px' : (pos[1] + dim.height - 1) + 'px';
...
with (Control.DatePicker)
{
    ...
    Locale['fr_iso8601'] = i18n.createLocale('iso8601', 'fr');
...
Object.extend(Control.DatePickerPanel.prototype, {
    initialize: function(options, nonDutyDays)
    {	this.i18n = new Control.DatePicker.i18n(options && options.locale ? options.locale : 'en_US');
        options = this.i18n.inheritOptions(options);
        this.options = Object.extend({
    ...
    createPicker: function()
    {
        ...
        if (this.options.captureKeys)
            this.captureKeys();
        // Need to use background iframe to workaround the ActiveX control and div overlap bug in IE6 and below
        if (/MSIE/.test(navigator.userAgent) && parseFloat(navigator.appVersion) <= 6)
        {
            this.appendBackgroundIframe(elt);
        }
        return elt;
    },
    tr: function(str)
    ...
    setDate: function(date)
    {
        if (date)
        {
            ...
            this.calendarCont.appendChild(this.createCalendar(date, null));
            // Add background iframe back for IE6 and below, as it was deleted in the above code
            if (/MSIE/.test(navigator.userAgent) && parseFloat(navigator.appVersion) <= 6)
            {
            	this.appendBackgroundIframe(this.element);
            }
        }
    },
    drawCalendar: function(container, date)
    {
        ...
        if (this.options.timePicker)
        {
            ...
            {
                if (e.keyCode === Event.KEY_RETURN && this.options.onSelect){
    ...
    movePreviousMonthListener: function()
    {
        return function()
        {
            ...
            if (prevMonth.getMonth() !== (this.currentDate.getMonth() + 11) % 12){ 
              prevMonth.setDate(0);
              }
    ...
    moveNextMonthListener: function()
    {
        return function()
        {
            var nextMonth = new Date(
                    ...
                    this.currentDate.getHours(),
                    this.currentDate.getMinutes());
            if (nextMonth.getMonth() !== (this.currentDate.getMonth() + 1) % 12) {
              nextMonth.setDate(0);
              }
    ...
    moveNextYearListener: function()
    {
        return function()
        {
            var nextYear = new Date(
                    ...
                    this.currentDate.getHours(),
                    this.currentDate.getMinutes());
            if (nextYear.getMonth() !== this.currentDate.getMonth()) {
              nextYear.setDate(0);
            }
    ...
    movePreviousYearListener: function()
    {
        return function()
        {
            var prevYear = new Date(
                    ...
                    this.currentDate.getHours(),
                    this.currentDate.getMinutes());
            if (prevYear.getMonth() !== this.currentDate.getMonth()) {
              prevYear.setDate(0);
            }
    ...
    assignDayClasses: function(cell, baseClass, date)
    {
        ...
        if (this.options.weekend.include(date.getDay())){
        	...
        	if(getNDutyDays() != null){
        		var numDays = getNDutyDays().length;
	    		for(var j=0; j<numDays; j++) {
	    			var d = getNDutyDays()[j];
	    			if(d != null){
		    			var nDate = new Date(d);
		    			if(date.getFullYear() === nDate.getFullYear() && date.getMonth() === nDate.getMonth() && date.getDate() === nDate.getDate()){
		    				Element.addClassName(cell, 'weekend');
		    			}
	    			}
	    		}
    ...
    appendBackgroundIframe: function(div)
    {
